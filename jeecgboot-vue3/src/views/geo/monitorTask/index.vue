<template>
  <div>
    ...
    <MonitorTaskModal @register="registerModal" @success="reload" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import MonitorTaskModal from './MonitorTaskModal.vue';
  import { columns, searchFormSchema } from './monitorTask.data';
  import { getMonitorTaskList, deleteMonitorTask, batchDeleteMonitorTask, runMonitorTask } from './monitorTask.api';
  ...
  function getActions(record) {
    return [
      ...
      { label: '立即执行', onClick: () => runMonitorTask({ id: record.id }, reload) },
  ...
  function handleDelete(record) { deleteMonitorTask({ id: record.id }, reload); }
  function batchHandleDelete() {
    batchDeleteMonitorTask({ ids: selectedRowKeys.value.join(',') }, () => {
