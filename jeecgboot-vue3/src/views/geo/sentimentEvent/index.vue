<template>
  <div>
    ...
    <SentimentEventModal @register="registerModal" @success="reload" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import SentimentEventModal from './SentimentEventModal.vue';
  import { columns, searchFormSchema } from './sentimentEvent.data';
  import { getSentimentEventList, deleteSentimentEvent, batchDeleteSentimentEvent } from './sentimentEvent.api';

  const selectedRowKeys = ref<Array<string | number>>([]);
  const [registerModal, { openModal }] = useModal();
  const [registerTable, { reload }] = useTable({
    title: 'GEO 舆情事件',
    api: getSentimentEventList,
    columns,
  ...
  function handleDelete(record) { deleteSentimentEvent({ id: record.id }, reload); }
  function batchHandleDelete() {
    batchDeleteSentimentEvent({ ids: selectedRowKeys.value.join(',') }, () => {
